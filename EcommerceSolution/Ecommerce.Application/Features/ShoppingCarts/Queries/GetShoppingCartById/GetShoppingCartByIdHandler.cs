...
{
    public class GetShoppingCartByIdHandler : IRequestHandler<GetShoppingCartByIdQuery, ShoppingCartVm>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GetShoppingCartByIdHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        public async Task<ShoppingCartVm> Handle(GetShoppingCartByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            var shoppingCart = await _unitOfWork.Repository<ShoppingCart>().GetEntityAsync(
                ...
                includes,
                true // que lo mantenga en memoria
            );
